...
namespace CryptoLibrary
{
    public class GOSTLib
    {
        ...
        {
            MemoryStream EncryptData(MemoryStream planeText);
            MemoryStream DecryptData(MemoryStream chiferedText);
        }
        public class GostProvider : IDataCrypt
        {
            public MemoryStream EncryptData(MemoryStream sourceData)
            {
                GOST28147_89 gost = new GOST28147_89();
                gost.SetOptimTable();
                byte[] encryptedData = gost.GOSTDataCrypt(sourceData.ToArray());
                MemoryStream encryptedDataStream = new MemoryStream(encryptedData);
                return encryptedDataStream;
            }
            public MemoryStream DecryptData(MemoryStream sourceData)
            {
                GOST28147_89 gost = new GOST28147_89();
                gost.SetOptimTable();
                byte[] decryptedData = gost.GOSTDataDecrypt(sourceData.ToArray());
